...
//
// When the backend supplies a nearestScenario, the section renders the real
// adjacent-pathway enrichment widget (the nearestScenario.value object) and
// prefixes it with SubstitutionNote, so the substitute looks like a served
// value but is clearly labeled as adjacent-scenario context.
...
// Banner shown above a substitute enrichment widget. The body below it is the
// real nearest-pathway value, so this label is what keeps it from being read
// as the selected scenario's value.
export function SubstitutionNote({
  ...
  return (
    <div style={{ marginBottom: 10, padding: "8px 11px", borderRadius: 8, border: `1px dashed ${accent}66`, background: `${accent}0c` }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 3 }}>
        ...
        {scenario && <span style={{ fontSize: 11, fontWeight: 700, color: "rgba(255,255,255,0.82)" }}>{scenario} pathway</span>}
      </div>
      <p style={{ margin: 0, fontSize: 11, lineHeight: 1.5, color: MUTED }}>
        {note ?? "Shown as adjacent-scenario context, not a value for the selected scenario."}
        {servedScenario ? ` No value is published for ${servedScenario}.` : ""}
      </p>
    </div>
  );
}
