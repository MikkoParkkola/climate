      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <header className="space-y-3">
          <nav className="flex flex-wrap gap-4 text-sm">
            ...
            <a href="/data-quality" className="text-[hsl(24,88%,66%)] hover:underline">Data quality</a>
          </nav>
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-amber-400">Bounded rankings</p>
        ...
        <section className="grid gap-3 rounded border border-[hsl(220,13%,22%)] bg-[hsl(222,15%,12%)] p-4 md:grid-cols-5">
          <label className="space-y-1 text-sm">
            <span className="font-medium text-slate-300">Catalog</span>
            <select
              value={catalog}
              onChange={(event) => setCatalog(event.target.value as (typeof CATALOGS)[number]["id"])}
              className="w-full rounded border border-[hsl(220,13%,22%)] bg-[hsl(222,15%,16%)] px-3 py-2"
            >
              {CATALOGS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
            </select>
            <span className="block text-xs leading-5 text-slate-400">{selectedCatalog.description}</span>
          </label>
          <label className="space-y-1 text-sm">
          ...
          <label className="space-y-1 text-sm">
            ...
            <select value={scenario} onChange={(event) => setScenario(event.target.value)} className="w-full rounded border border-[hsl(220,13%,22%)] bg-[hsl(222,15%,16%)] px-3 py-2">
          ...
          <label className="space-y-1 text-sm">
            <span className="font-medium text-slate-300">Year</span>
            <select value={year} onChange={(event) => setYear(Number(event.target.value))} className="w-full rounded border border-[hsl(220,13%,22%)] bg-[hsl(222,15%,16%)] px-3 py-2">
              {YEARS.map((item) => <option key={item} value={item}>{item}</option>)}
            </select>
          </label>
          <label className="space-y-1 text-sm">
            <span className="font-medium text-slate-300">Direction</span>
            <select value={direction} onChange={(event) => setDirection(event.target.value as Direction)} className="w-full rounded border border-[hsl(220,13%,22%)] bg-[hsl(222,15%,16%)] px-3 py-2">
        ...
        {isLoading && <Card><CardContent className="py-6 text-sm text-slate-400">Loading ranking artifact...</CardContent></Card>}
        {error && <Card><CardContent className="py-6 text-sm text-red-400">No precomputed ranking is available for that combination.</CardContent></Card>}
            ...
            <section className="grid gap-3 sm:grid-cols-3">
              <div className="rounded border border-[hsl(220,13%,22%)] bg-[hsl(222,15%,16%)] px-3 py-2">
                ...
                {Number.isFinite(data.placeSampleSize) && (
                  <div className="text-xs text-slate-400">from {Number(data.placeSampleSize).toLocaleString()} included populated places</div>
                )}
              </div>
              <div className="rounded border border-[hsl(220,13%,22%)] bg-[hsl(222,15%,16%)] px-3 py-2">
                <div className="text-xs uppercase tracking-wide text-slate-400">Metric</div>
              ...
              <CardContent className="space-y-4 pt-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    <BarChart3 className="h-5 w-5 text-[hsl(24,88%,66%)]" aria-hidden />
                  ...
                  <a
                    href={query}
                    className="inline-flex items-center gap-1 text-sm font-medium text-[hsl(24,88%,66%)] hover:underline"
                  >
                ...
                <div className="overflow-x-auto rounded border border-[hsl(220,13%,22%)]">
                  <table className="w-full min-w-[760px] text-left text-sm">
                    <thead className="bg-[hsl(222,15%,16%)] text-slate-400">
                      <tr>
                    ...
                    <tbody>
                      {data.rows.map((row) => (
                        <tr key={row.id} className="border-t border-[hsl(220,13%,22%)]">
                          <td className="px-3 py-2 font-semibold">#{row.rank}</td>
                          <td className="px-3 py-2">
                            <div className="font-medium">
                              {row.country === "country aggregate" ? row.name : `${row.name}, ${row.country}`}
                            </div>
                            <div className="text-xs text-slate-400">{row.lat.toFixed(2)}, {row.lng.toFixed(2)}</div>
                            ...
                            {row.inclusionReason && (
                              <div className="max-w-md text-xs text-slate-400">{row.inclusionReason}</div>
                            )}
                          </td>
                          <td className="px-3 py-2 font-medium">{formatValue(row.value, row.unit)}</td>
                          <td className="px-3 py-2 text-slate-400">{uncertaintyLabel(row)}</td>
                          <td className="px-3 py-2 text-xs text-slate-400">{row.sourceReceipt.join(", ")}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <Card>
                <CardContent className="space-y-3 pt-6">
                  ...
                  <dl className="space-y-2 text-sm">
                    ...
                    <div><dt className="font-medium text-slate-400">Source IDs</dt><dd className="break-words">{data.sourceIds.join(", ")}</dd></div>
                    <div><dt className="font-medium text-slate-400">Exclusions</dt><dd>{data.exclusions.length ? data.exclusions.join("; ") : "None in this bounded catalog slice."}</dd></div>
                  </dl>
