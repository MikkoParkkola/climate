...
import { Thermometer, Droplets, MapPin, TrendingUp, TrendingDown, AlertTriangle, Waves, Cloud, FileText, Globe, Award } from "lucide-react";
                        ...
                        <div className="relative">
                          ...
                          {(() => {
                            ...
                            return (
                              <div className="space-y-2">
                                {/* Chart container */}
                                <div className="relative h-64 flex items-end justify-between px-4 border-b border-gray-300">
                                  {/* Zero line */}
                                  <div className="absolute bottom-16 left-0 right-0 h-px bg-gray-400 z-10"></div>
                                  <div className="absolute bottom-14 left-2 text-xs text-gray-500">0</div>
                                  
                                  {/* Bars */}
                                  {chartData.map((item, index) => {
                                    const barHeight = Math.abs(item.value) / maxValue * 160; // 160px max height
                                    const bottomOffset = (item.startValue / maxValue * 160) + 64; // 64px from bottom for labels
                                    
                                    return (
                                      <div key={index} className="relative flex flex-col items-center" style={{ width: '14%' }}>
                                        {/* Connecting line from previous bar */}
                                        {index > 0 && (
                                          <div 
                                            className="absolute border-t-2 border-dashed border-gray-400 z-20"
                                            style={{
                                              width: '100%',
                                              top: `${240 - bottomOffset}px`,
                                              left: '-50%'
                                            }}
                                          ></div>
                                        )}
                                        ...
                                        {/* Component name */}
                                        <div className="absolute bottom-8 text-xs text-center font-medium text-gray-700 px-1 leading-tight">
                                          {item.name}
                                        </div>
                                      </div>
                                    );
                                  ...
                                  <div className="relative flex flex-col items-center" style={{ width: '14%' }}>
                                    <div
                                      className="bg-purple-600 relative border border-gray-300 font-bold"
                                      style={{
                                        height: `${(finalScore / maxValue) * 160}px`,
                                        width: '100%',
                                        position: 'absolute',
                                        bottom: '64px'
                                      }}
                                    ...
                                    <div className="absolute bottom-8 text-xs text-center font-bold text-purple-700 px-1">
                                      Final Score
                                    </div>
                                  </div>
                                </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-2">
                        {climateData.time_series.years?.map((year: number, index: number) => {
                          ...
                          const habitChange = prevHabit ? habit - prevHabit : null;
                          ...
                          return (
                            <div key={year} className={`p-3 rounded-lg border ${isTarget ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
                              ...
                              {/* Show breakdown changes for target year */}
                              {isTarget && habitChange !== null && Math.abs(habitChange) > 0.5 && (
                                <div className="mt-2 p-2 bg-white rounded border border-gray-200">
                                  <div className="text-xs font-medium text-gray-700 mb-1">
                                    Habitability change breakdown since {year - 5}:
                                  </div>
                                  <div className="grid grid-cols-2 gap-1 text-xs">
                                    <div className={`${habitChange > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                      • Temperature comfort: {habitChange > 0 ? '+' : ''}{(habitChange * 0.4).toFixed(1)}
                                    </div>
                                    <div className={`${habitChange > 0 ? 'text-green-600' : 'text-red-600'}`}>
                                      • Precipitation adequacy: {habitChange > 0 ? '+' : ''}{(habitChange * 0.3).toFixed(1)}
                                    </div>
                                    <div className={`${habitChange < 0 ? 'text-orange-600' : 'text-gray-600'}`}>
                                      • Heat stress penalty: {habitChange < 0 ? '+' : ''}{Math.abs(habitChange * 0.2).toFixed(1)}
                                    </div>
                                    <div className={`${habitChange < 0 ? 'text-yellow-600' : 'text-gray-600'}`}>
                                      • Drought/flood risks: {habitChange < 0 ? '+' : ''}{Math.abs(habitChange * 0.1).toFixed(1)}
                                    </div>
                                  </div>
                                </div>
                              )}
              ...
              <Card className="border-purple-200">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-purple-700">
                    ...
                    Habitability Breakdown Analysis
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <LivabilityIndexBreakdown 
                    projectedData={{
                      id: 1,
                      locationId: 1,
                      projectionYear: climateData.year,
                      habitabilityScore: climateData.habitability?.score,
                      habitabilityBreakdown: climateData.habitability?.breakdown
                    }}
                    selectedYear={climateData.year}
                    globalPercentile={(() => {
                      // Calculate global percentile based on current score
                      const currentScore = climateData.habitability?.score || 0;
                      // Use a representative global distribution for percentile calculation
                      if (currentScore >= 75) return 90;
                      if (currentScore >= 65) return 75;
                      if (currentScore >= 55) return 60;
                      if (currentScore >= 45) return 40;
                      if (currentScore >= 35) return 25;
                      return 10;
                    })()}
                  />
                </CardContent>
