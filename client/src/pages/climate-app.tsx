            ...
            <CardContent className="space-y-8">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative bg-gray-50 rounded-lg p-4 h-64">
                      {(() => {
                        ...
                        // Calculate baseline temperatures (2025) by subtracting the climate change
                        const baselineTemps = monthlyTemps.map((temp: number) => temp - tempChange);
                        
                        // Calculate scale including both baseline and projected
                        const allTemps = [...monthlyTemps, ...baselineTemps];
                          ...
                          <>
                            ...
                            <div className="absolute left-14 right-4 top-4 bottom-16">
                              ...
                              <svg className="w-full h-full" viewBox="0 0 100 100" preserveAspectRatio="none">
                                {/* Baseline temperature line (2025) */}
                                <polyline
                                  fill="none"
                                  stroke="#6b7280"
                                  strokeWidth="2"
                                  ...
                                  points={baselineTemps.map((temp: number, index: number) => {
                            ...
                            <div className="absolute top-1 right-4 text-xs">
                              <div className="flex items-center gap-4 bg-white/90 p-2 rounded border">
                                <div className="flex items-center gap-1">
                                  <div className="w-4 h-0.5 bg-gray-500" style={{borderStyle: 'dashed', borderWidth: '1px 0'}}></div>
                                  <span className="text-gray-600">2025 Baseline</span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <div className="w-4 h-0.5 bg-red-600"></div>
                                  <span className="text-red-600">{climateData.year} Projected</span>
                                </div>
                              </div>
                            </div>
                            
                            {/* Value explanation labels */}
                            <div className="absolute bottom-20 left-14 right-4 text-xs bg-white/90 p-1 rounded border">
                              <div className="flex justify-between items-center">
                                <span className="text-gray-500">Baseline (2025)</span>
                                <span className="text-red-600">Projected ({climateData.year})</span>
                                <span className="text-gray-400">Change (Δ)</span>
                              </div>
                            </div>
                            
                            {/* Month labels with temperature values and differences */}
                            <div className="absolute bottom-0 left-14 right-4 h-16">
                              <div className="relative w-full h-full">
                                {['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'].map((month, index) => {
                                  const projectedTemp = monthlyTemps[index];
                                  const baselineTemp = baselineTemps[index];
                                  const difference = projectedTemp - baselineTemp;
                                  ...
                                  return (
                                    ...
                                    >
                                      ...
                                      <div className="text-gray-500 font-mono text-xs">
                                        {baselineTemp?.toFixed(1)}°C
                                      </div>
                                      <div className={`font-mono text-xs font-bold ${projectedTemp >= 0 ? 'text-red-600' : 'text-blue-600'}`}>
                                        {projectedTemp?.toFixed(1)}°C
                                      </div>
                                      <div className={`font-mono text-xs font-bold ${difference > 0 ? 'text-red-500' : difference < 0 ? 'text-blue-500' : 'text-gray-500'}`}>
              ...
              <Card className="border-blue-200">
                ...
                <CardContent className="space-y-4">
                  ...
                  {/* Monthly Precipitation Chart with Baseline Comparison */}
                  <div className="space-y-4">
                    <h4 className="font-medium">Monthly Precipitation Distribution</h4>
                    <div className="relative">
                      {/* Legend */}
                      <div className="absolute top-0 right-0 text-xs mb-2">
                        <div className="flex items-center gap-4 bg-white/90 p-2 rounded border">
                          <div className="flex items-center gap-1">
                          ...
                          <div className="flex items-center gap-1">
                            <div className="w-4 h-2 bg-gradient-to-t from-blue-600 to-blue-300 rounded"></div>
                            <span className="text-blue-600">{climateData.year} Projected</span>
                          </div>
                        </div>
                      </div>
                      
                      {/* Value explanation labels */}
                      <div className="absolute top-20 left-0 right-0 text-xs bg-white/90 p-1 rounded border mx-4">
                        <div className="flex justify-between items-center">
                          <span className="text-gray-500">Baseline (2025)</span>
                          <span className="text-blue-600">Projected ({climateData.year})</span>
                          <span className="text-gray-400">Change (Δ)</span>
                        </div>
                      </div>
                      
                      <div className="grid grid-cols-12 gap-1 text-xs h-32 mb-4 mt-12">
                        {climateData.precipitation?.monthly?.map((precip: number, index: number) => {
                          const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
                          const precipChange = climateData.precipitation_change || 0;
                          
                          // Calculate baseline precipitation (2025) by removing the change
                          const baselinePrecip = precip / (1 + precipChange / 100);
                          
                          // Use maximum of both baseline and projected for scaling
                          const allPrecipValues = climateData.precipitation.monthly.map((p: number) => {
                            const baseline = p / (1 + precipChange / 100);
                            return Math.max(p, baseline);
                          });
                          const maxPrecip = Math.max(...allPrecipValues);
                          
                          const projectedHeight = Math.max(8, (precip / maxPrecip) * 96);
                          const baselineHeight = Math.max(8, (baselinePrecip / maxPrecip) * 96);
                          const difference = precip - baselinePrecip;
                          
                          return (
                            <div key={index} className="text-center flex flex-col justify-end h-full relative">
                              {/* Side-by-side bars container */}
                              <div className="flex items-end justify-center gap-0.5 mb-12 h-24">
                                {/* Baseline bar (left) */}
                                <div 
                                  className="bg-gray-400 rounded-t w-5 opacity-70"
                                  style={{ height: `${baselineHeight}px` }}
                                  title={`${months[index]} Baseline: ${baselinePrecip.toFixed(1)}mm`}
                                ></div>
                                
                                {/* Projected bar (right) */}
                                <div 
                                  className="bg-gradient-to-t from-blue-600 to-blue-300 rounded-t w-5"
                                  style={{ height: `${projectedHeight}px` }}
                                  title={`${months[index]} Projected: ${precip.toFixed(1)}mm`}
                                ></div>
                              </div>
                              
                              <div className="text-xs text-gray-600 font-medium">{months[index]}</div>
                              <div className="text-xs font-mono text-gray-500">
                                {baselinePrecip.toFixed(0)}mm
                              </div>
                              <div className="text-xs font-mono text-blue-700 font-bold">
                                {precip.toFixed(0)}mm
                              </div>
                              <div className={`text-xs font-mono font-bold ${difference > 0 ? 'text-green-600' : difference < 0 ? 'text-orange-600' : 'text-gray-500'}`}>
                                {difference > 0 ? '+' : ''}{difference.toFixed(0)}mm
                              </div>
                            </div>
                          );
                        })}
                        ...
                        <div className="relative">
                          ...
                          {(() => {
                            ...
                            const components = [
                              ...
                              ...(breakdown.nordic_climate_bonus ? [{ name: 'Nordic Climate Bonus', value: breakdown.nordic_climate_bonus, color: 'bg-purple-500' }] : [])
                            ];
