...
> This doc covers the **next chapter** of grounding fupit: filling the habitability signals still marked `partial`,
...
2. **Project is NON-COMMERCIAL** (operator, 2026-06-29). License bar = non-commercial use +
   ...
   `NC-ONLY`, which is fine: fupit is and remains a non-commercial, educational project.
...
### E1 — NASA NEX-GDDP-CMIP6 (closes 3 gaps) — HIGHEST IMPACT
...
- **License:** **CC0** (verified verbatim on AWS registry, blanket since Sept 2022). Public domain,
  so it carries no reuse restrictions at all. The one pick with zero license risk. **[V]**
...
- **Replit republish + live verification.** Landed code (freshwater, dark-theme, visual polish)
  is on `main` but not yet live; users see the old site.
