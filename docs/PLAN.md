...
- **Model consensus vs IPCC-calibrated**: show both numbers + the adjustment, default to the raw CMIP6 model consensus, so the "hot model" tuning is visible rather than hidden (operator decision 2026-06-27; method in `architecture/SCIENTIFIC_GROUNDING.md`).
...
- [~] Phase 4 — serving layer refactor. **Engine swapped:** the forecast request path now uses the TypeScript in-process grid engine in `server/grounded-node-model.ts` exclusively; the runtime Python subprocess and the `CLIMATE_GRID_ENGINE` switch have been removed. `grounded_model.py` no longer serves any request — it remains only as the offline ranking-artifact builder and the parity oracle the Node engine is tested against (`npm run smoke:grid-reader`, `smoke:node-model`). API-key gates were removed (engine is offline). The Node engine reads `data/grid.i16.gz` and observed-baseline artifacts through the TypeScript grid reader — no xarray/netcdf at serve time, no NVIDIA API, no `child_process` spawn in the serving path. Final grid export is in `data/`; direct engine smoke, production build, `npm run ci`, local Postgres-backed endpoint smoke, Node grid decode/interpolation parity, full projected-response parity, and warm Node trajectory performance (p50 ~1ms) are green. Location search is now resolved by the free Open-Meteo geocoding API (any place on Earth, no DB dependency, graceful fallback). `climate_model_cache` identity now includes scenario, cache version, and source-registry version, so old unversioned cbottle-era rows read as misses; server startup deletes incompatible rows. Source registry, formal `GET /api/climate-twin`, curated-city rankings, and Natural Earth population-place ranking artifacts are served from static data, not request-time global Python loops. Scenario selector, projection uncertainty receipts, and bounded climate-twin catalog receipts are implemented on the main result flow; comparison mode also has an explicit scenario selector and a regression guard for the year-slider overlap bug. **Remaining:** Replit autoscale republish, production cache purge/version-guard proof, live verification, live screenshot capture, and final live endpoint/performance proof on Replit-class infrastructure.
- [x] Phase 5 (partial) — `/methodology` public page shipped (`client/src/pages/methodology.tsx`, route in `App.tsx`), documents full grounding stack + exact risk threshold bands. `/rankings` now exposes bounded top-10 lists from the curated-city artifact, a Natural Earth 1:110m population-place catalog (`pop_max >= 3,000,000`), and a Natural Earth-derived country aggregate catalog weighted by included populated-place `pop_max`; each catalog exposes catalog size, source receipts, caveats, and no safe-city/global-complete claims. `/data-quality` exposes packaged artifact hashes, a visible source/license registry table with method, coverage, license, commercial reuse, redistribution, display policy, and the unregistered-source suppression rule, default-policy source IDs for the current-policy reference, ranking-catalog coverage, trajectory-audit coverage, trend-review flags, validation-report status, and launch limitations. The main result receipt now offers copy/download raw forecast JSON with selected-year point, full trajectory, scenario, model metadata, uncertainty fields, and source trail; `npm run smoke:raw-json` guards that surface. It also offers a Markdown educational summary for classroom, journalist, civic, and research reuse; the report is built from the same visible forecast fields, annual roadmap, climate twin, source trail, and missing-domain caveats, and `npm run smoke:educational-report` guards that it does not add unregistered enrichments or safe-city claims. The result page now includes a guarded "why this changed" score-component ranking, per-decade trend rates, grounded low-high uncertainty bands on temperature/precipitation/sea-level trend charts, annual interpolated trend points with hover values and keyboard/touch values tables, explicit scenario labels on each metric trend chart, seven core trajectory charts including flood risk, labeled threshold markers with accessible threshold receipts where documented, a current-year-to-2100 living-conditions roadmap with scenario deltas when contrast is loaded, keyboard/touch-accessible source receipts on the headline KPIs, daily-life rows, and risk/extremes cards, raw ETCCDI values beside drought/flood/heat risk scores, daily-life interpretation, explicit missing-domain caveats, registered AMOC/Gulf Stream regional tail-risk context where relevant without local correction, and sea-level wording that uses a source-registered Natural Earth 1:110m nearest-coast screen for coarse relevance while still rejecting parcel-level elevation/exposure claims; `npm run smoke:storyline` guards that surface. The result page now has a shareable climate story card that packages the selected-year metrics, top visible score driver, climate-twin analog, platform share API, clipboard fallback, downloadable PNG social image, grounded follow-up comparison prompts, and caveats against unregistered enrichments or safe-city claims; `npm run smoke:share-card` guards the R10 share surface. The result page also has an on-demand same-location scenario contrast for lower-warming, current-policy-adjacent, high, and stress pathways with an accessible method receipt and educational, non-preachy call-to-action framing; `npm run smoke:scenario-contrast` guards the R7 pathway framing and caveats. `docs/VALIDATION_REPORT.md` is generated by `npm run report:validation` and guarded by `npm run smoke:validation-report`; it proves the fixture trajectory contract, includes a 13-city WorldClim observed-baseline cross-check, and now adds a NASA POWER/MERRA-2 external observed-climatology baseline comparison for the same fixture cities. That external baseline check is not a correction layer and not proof of future projection skill, so the report explicitly keeps a true time-varying projection-vs-observation hindcast pending. `npm run validate:artifacts` also verifies the current climate-twin analog catalog, observed-baseline audit, NASA POWER observed-climatology validation artifact, ranking artifacts, coastal-proximity artifact, and registered source rows. `client/public/llms.txt` refreshed for grounded fupit pages. `npm run screenshots:capture` provides a Chrome-based desktop/mobile screenshot harness for the layered launch check. **Remaining:** true time-varying projection-vs-observation hindcast comparison, GHSL/full population-weighted exposure ranking artifacts, and live screenshot evidence.
  - Scenario contrast now includes all-key-metric small multiples for raw/IPCC warming, heat-stress days, rainfall change, drought risk, flood risk, sea-level context, and habitability, with annual interpolation and selected-year value receipts guarded by `npm run smoke:scenario-contrast`.
  - Score sensitivity now exposes the visible habitability breakdown as an interactive what-if: users can hide components or adjust component multipliers, while the panel discloses the clamp formula and missing-domain limits. Guarded by `npm run smoke:storyline`.
  - Guided explainer mode now walks result-page users through trend-first reading, daily-life questions, score drivers, pathway/twin comparison, evidence limits, and classroom prompts using already visible grounded fields. Guarded by `npm run smoke:guided-explainer`.
  - Cold-season context now counts selected-year monthly mean temperature months at or below 0°C and surfaces that proxy in daily-life rows, risk/extremes cards, the annual roadmap, the educational report, and `/methodology`. It is explicitly caveated as not daily freeze days, daily cold-stress days, freeze-thaw, heating demand, road/crop damage, pests, or health risk. Guarded by `npm run smoke:storyline` and `npm run smoke:educational-report`.
