...
- [~] Phase 4 — serving layer refactor. **Engine swapped:** `grounded_model.py` replaces `cbottle_runner.py` at all 3 spawn sites in `server/routes.ts`; API-key gates removed (engine is offline). Reads `data/grid.i16.gz` with numpy+gzip only — no xarray/netcdf at serve time, no NVIDIA API. Final grid export is in `data/`; direct engine smoke and production build are green. **Remaining:** HTTP endpoint smoke on a host with Neon/Replit-compatible `DATABASE_URL`; Replit autoscale republish; purge stale `climate_model_cache` on deploy (BLOCKING — old rows are fabricated cbottle output); scenario selector + uncertainty UI (deferred to v1.1).
...
2. Against the production Replit Postgres, run `TRUNCATE climate_model_cache;`. Last observed read-only check on 2026-06-27 still showed 8 rows, newest `2026-06-26 00:11:27 UTC`.
