...
- Production cache purge remains required after any change to the projection engine or model version unless a version guard has been proven live on public `/api/health`.
...
5. Verify no legacy projection route serves ungrounded output.
...
- Cache contains no stale payloads from an older model version, or they are rejected by live version guard.
