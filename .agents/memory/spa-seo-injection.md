...
**Dev preamble gotcha (cost ~half a day of "React won't mount"):** the *dev* branch of this middleware (`buildDevHtml` in `server/routes.ts`) hand-builds the HTML string itself instead of going through Vite's `transformIndexHtml`. Hand-built dev HTML that loads `/src/main.tsx` MUST also include `<script type="module" src="/@vite/client">` and the `@vitejs/plugin-react` refresh preamble (`import RefreshRuntime from "/@react-refresh"; ...; window.__vite_plugin_react_preamble_installed__ = true`) *before* the entry module — otherwise the plugin throws "can't detect preamble", React never mounts, and the page shows only the `<noscript>` fallback. Prod is unaffected (it reads the already-transformed built `index.html`).
**Why:** only Vite's `transformIndexHtml` injects the preamble automatically; any HTML you assemble by hand bypasses it.
